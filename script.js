...
const CHAINS = {
  1: { // Ethereum
    ...
    usdtDecimals: 6,
    params: null // switch için sadece 0x1 yeterli
  },
  56: { // BNB Smart Chain
    hex: "0x38",
    name: "BNB Chain",
...
/* ====== PART 2/4 – BUY FLOW (MetaMask + USDT transfer + receipts) ====== */

/** Admin (GELİRİ ALACAK) cüzdan adresi */
const ADMIN_USDT_RECEIVER = "0x69014a76Ee25c8B73dAe9044dfcAd7356fe74bC3";

/** USDT kontratları (mainnet) ve decimals: */
const USDT = {
  ...
  POLY: { address:"0xC2132D05D31c914a87C6611C10748AaCbC532",   decimals:6  }
};

/** Ağ objeleri (chain switch için) */
const ZUZU_CHAINS = {
  ETH: {
    chainId: '0x1',
    chainName: 'Ethereum Mainnet',
...
/** MetaMask bağla (hesap döndürür) */
async function connectMetaMask(){
  if(!window.ethereum){ alert('MetaMask not detected'); throw new Error('no_metamask'); }
...
/** Ağı değiştir/ekle */
async function zuzuSwitchChain(chainKey){
  const cfg = ZUZU_CHAINS[chainKey];
  if(!cfg){ alert('Unknown chain'); return false; }
  try{
    await window.ethereum.request({
      method: 'wallet_switchEthereumChain',
...
/** on-chain miktar string -> BigInt (decimals dikkate alarak) */
function toUnitsBigInt(amountStr, decimals){
  // "1234.56" → BigInt(123456000...) decimals göre
  amountStr = String(amountStr).trim();
  if(!amountStr.includes(".")) return BigInt(amountStr) * (BigInt(10)**BigInt(decimals));
  const [i,f] = amountStr.split(".");
  const frac = (f + "0".repeat(decimals)).slice(0, decimals);
  const bi  = BigInt(i||"0") * (BigInt(10)**BigInt(decimals));
  const bf  = BigInt(frac||"0");
  return bi + bf;
}

/** ERC20 transfer data: a9059cbb + padded(to) + padded(value) */
function buildERC20TransferData(to, valueBigInt){
  function padTo32Bytes(hex){
    return hex.replace("0x","").padStart(64, "0");
  }
  const method = "0xa9059cbb";
  const toHex  = padTo32Bytes(to.toLowerCase().replace("0x",""));
  const valHex = padTo32Bytes("0x" + valueBigInt.toString(16));
  return method + toHex + valHex;
}

/** USDT transfer et (kullanıcı cüzdanından ADMIN_USDT_RECEIVER’a) */
async function sendUSDT(chainKey, usdtAmount){
  if(!window.ethereum){ alert("MetaMask not detected"); return; }
  const account = await connectMetaMask();
  const ok = await zuzuSwitchChain(chainKey);
  if(!ok) return;

  const token = USDT[chainKey];
  if(!token){ alert("USDT mapping not found for chain"); return; }

  const value = toUnitsBigInt(usdtAmount, token.decimals);
  const data  = buildERC20TransferData(ADMIN_USDT_RECEIVER, value);
  ...
  return { txHash, account, chainKey, usdtAmount };
}

/** Satın alma fişi (localStorage’a yaz) */
function saveReceipt(rec){
  const k = "zuzu_receipts";
...
/** Buy butonları: hesapla ve transfer et */
(function initBuyButtons(){
  ...
  btns.forEach(b=>{
    b.addEventListener("click", async ()=>{
      try{
        const week   = parseInt(b.getAttribute("data-week"), 10) || 0;
        const qtyEl  = document.getElementById("buyAmount");
        const chain  = (document.getElementById("buyChain")?.value || "BSC");
        const qty    = parseFloat((qtyEl?.value||"0").toString().replace(/[^\d.]/g,"")) || 0;

        const price  = CONFIG.weekPrices[week] || CONFIG.weekPrices[0];
        const cost   = qty * price; // USDT cinsinden

        if(cost <= 0){ alert("Please enter a valid amount."); return; }
        if(!window.ethereum){ alert("MetaMask not detected"); return; }

        // Transfer
        const rec = await sendUSDT(chain, cost.toFixed(6));
        if(rec && rec.txHash){
          saveReceipt(rec);
          alert("Purchase successful!\nTx: "+rec.txHash+"\nUSDT: "+rec.usdtAmount+"\nNetwork: "+rec.chain);
        }
      }catch(err){
        console.warn("buy flow error", err);
