...
/* =========================
   MetaMask – Connect & Buy (STABLE)
========================= */
let provider, signer, currentAccount = null, currentChainId = null;

async function getProvider(){
  if (!window.ethereum || !window.ethereum.isMetaMask) {
    alert("MetaMask not detected. Install MetaMask and reopen the page.");
    // İsteğe bağlı: kullanıcıyı MetaMask install sayfasına yönlendirelim
    try { window.open("https://metamask.io/download/", "_blank"); } catch(e){}
...
function bindMetaMaskListeners(){
  if (!window.ethereum || window.ethereum._zuzuBound) return;
  window.ethereum.on("accountsChanged", (accs)=>{
    const b = document.getElementById("connectBtn");
    if (accs && accs.length){
      currentAccount = accs[0];
      signer = provider.getSigner();
      if (b) b.textContent = `MetaMask ${currentAccount.slice(0,6)}...${currentAccount.slice(-4)}`;
    } else {
      currentAccount = null; signer = null;
      if (b) b.textContent = "Connect Wallet";
    }
  });
  window.ethereum.on("chainChanged", (hexId)=>{
    // Sayfayı yenilemeyelim; local state’i güncelleyelim
    try { currentChainId = parseInt(hexId, 16); } catch(e) {}
  });
...
async function connectWallet(){
  ...
  if (btn) btn.textContent = `MetaMask ${currentAccount.slice(0,6)}...${currentAccount.slice(-4)}`;
...
function getSelectedChainId(){
  ...
  return [1,56,137].includes(v) ? v : 56; // default BSC
}
...
async function ensureChain(targetId){
  ...
  if (net.chainId === targetId) return;
  ...
  try{
    ...
    // 4902 → kullanıcı c-chain'i eklememiş, ekleyelim
...
async function payUSDT(costFloat){
  // costFloat: USDTTutarı (ör. 12.3456)
  await getProvider();
  await connectIfNeeded();

  const cid = getSelectedChainId();
  await ensureChain(cid);

  const meta = CHAINS[cid];
  // signer ile contract: msg.sender = currentAccount
  const token = new ethers.Contract(meta.usdt, ERC20_ABI, signer);

  // USDT decimals’a göre parse
  const dec = meta.usdtDecimals;
  const amtStr = Number.isFinite(costFloat)
    ? costFloat.toFixed(dec > 6 ? 6 : dec)
    : "0";
  ...
  // Transfer yap (ERC20.transfer: sender → ownerAddress)
...
async function handleBuy(weekIndex){
  try {
    ...
    const price = CONFIG.weekPrices[weekIndex]; // USDT per ZUZU
    const cost  = qty * price;

    const txHash = await payUSDT(cost);
    alert(`Purchase successful!\nTX: ${txHash}\nYou can claim later from Claim Portal.`);
  } catch(e){
    console.error(e);
    if (e && e.code === 4001) alert("You rejected the transaction in MetaMask.");
...
document.getElementById("networkSel")?.addEventListener("change", async ()=>{
  try { await ensureChain(getSelectedChainId()); } catch(e){ console.warn(e); }
});
