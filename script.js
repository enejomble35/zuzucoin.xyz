/* =========================
   ZUZU – Global Config (Persisted launch/sale)
========================= */

// Persisted 50-day countdown (doesn't reset on refresh)
function getPersistedLaunchAt() {
  try {
    const k = 'zuzu_launchAt';
    const saved = localStorage.getItem(k);
    if (saved) {
      const t = parseInt(saved, 10);
      if (isFinite(t) && t > Date.now()) return t;
    }
    const next = Date.now() + 50 * 24 * 60 * 60 * 1000; // 50 days
    localStorage.setItem(k, String(next));
    return next;
  } catch(e) {
    return Date.now() + 50 * 24 * 60 * 60 * 1000;
  }
}

// Persisted sale start (for active-week calc)
function getPersistedSaleStart() {
  try {
    const k = 'zuzu_saleStart';
    const saved = localStorage.getItem(k);
    if (saved) {
      const t = parseInt(saved, 10);
...
const CONFIG = {
  ...
  launchAt: getPersistedLaunchAt(),
  saleStart: getPersistedSaleStart(),
...
const CHAINS = {
  1: { // Ethereum
    ...
    usdtDecimals: 6,
    addParams: null // only switch needed
  },
  56: { // BNB Chain
    hex: "0x38",
    name: "BNB Chain",
...
/* =========================
   Countdown (uses persisted launchAt)
========================= */
function tick(){
...
/* =========================
   Active week (uses persisted saleStart)
========================= */
...
/* =========================
   MetaMask – Provider / Connect / Chain switch
========================= */
...
async function ensureProvider(){
  // ethers.js UMD must be included in index (already included)
  if (!window.ethereum) {
    // Mobile fallback deeplink
    if (isMobile()) {
      const dapp = `${location.host}${location.pathname}`.replace(/\/+$/,'');
      const link = `https://metamask.app.link/dapp/${dapp}`;
      alert("MetaMask not found. On mobile, open with MetaMask browser:\n" + link);
    } else {
  ...
  try {
    ...
    if (window.ethereum && !window.ethereum._zuzuBound) {
      ...
      window.ethereum.on("chainChanged", (_chainId)=>{
        // full reload to refresh provider/network
        location.reload();
      });
...
async function switchNetwork(targetId){
  ...
  try {
    ...
    // 4902 => add chain
    if (err && err.code === 4902 && meta.addParams) {
      await window.ethereum.request({
...
async function usdtTransfer(chainId, to, amountFloat){
  ...
  // Format amount to proper decimals (avoid too many decimals)
  const dec = meta.usdtDecimals;
  const amtStr = (Math.max(0, amountFloat)).toFixed(Math.min(dec, 6)); // up to 6 decimals
  ...
  try {
    ...
    console.warn("balance check failed (non-standard token?)", e);
  }
