...
// Zincir/USDT adresleri ve ondalıklar (mainnet)
const CHAINS = {
  1: { // Ethereum
    hex: "0x1",
    name: "Ethereum",
  ...
  56: { // BNB Smart Chain
    hex: "0x38",
    name: "BNB Chain",
    usdt: "0x55d398326f99059fF775485246999027B3197955", // 18 decimals (BSC)
    usdtDecimals: 18,
    params: {
      chainId: "0x38",
  ...
  137: { // Polygon
    hex: "0x89",
    name: "Polygon",
    usdt: "0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174", // 6 decimals (Polygon USDT)
    usdtDecimals: 6,
    params: {
      chainId: "0x89",
...
// Basit ERC20 ABI (transfer/balance/decimals)
const ERC20_ABI = [
...
const I = {
  en: {
    nav_presale: "Pre-Sale", nav_stake: "Stake", nav_nft: "NFT Rewards", nav_roadmap: "Roadmap", nav_token: "Tokenomics", connect: "Connect Wallet",
    ...
    noMetaMask: "MetaMask not detected. Please install MetaMask and refresh the page.",
    connectionFailed: "Connection failed or rejected. Please try again.",
    insufficientBalance: "Insufficient USDT balance.",
    networkSwitchFailed: "Network switch failed. Please switch manually.",
    invalidAmount: "Please enter a valid amount.",
    inactiveWeek: "This week is not active for purchase.",
    lowGas: "Insufficient gas (native token) balance.",
    purchaseSuccess: "Purchase successful!\nTX: {{txHash}}\nYou can claim later from Claim Portal."
  },
  tr: {
    nav_presale: "Ön Satış", nav_stake: "Stake", nav_nft: "NFT Ödülleri", nav_roadmap: "Yol Haritası", nav_token: "Tokonomi", connect: "Cüzdan Bağla",
    ...
    noMetaMask: "MetaMask bulunamadı. Lütfen MetaMask'i kurun ve sayfayı yenileyin.",
    connectionFailed: "Bağlantı başarısız oldu veya reddedildi. Lütfen tekrar deneyin.",
    insufficientBalance: "Yetersiz USDT bakiyesi.",
    networkSwitchFailed: "Ağ değiştirme başarısız. Lütfen manuel olarak değiştirin.",
    invalidAmount: "Lütfen geçerli bir miktar girin.",
    inactiveWeek: "Bu hafta satın alma için aktif değil.",
    lowGas: "Yetersiz gaz (yerel token) bakiyesi.",
    purchaseSuccess: "Satın alma başarılı!\nTX: {{txHash}}\nDaha sonra Claim Portal'dan talep edebilirsiniz."
  }
  // fr ve es için çeviriler eklenebilir
};
...
/* =========================
   Hata Mesajları
========================= */
function showError(message, lang = document.getElementById("langSel")?.value || "en") {
  const errorModal = document.getElementById("errorModal");
  if (!errorModal) return;
  const msg = message.includes("{{txHash}}") ? I[lang][message.split("{{txHash}}")[0]].replace("{{txHash}}", message.split("{{txHash}}")[1]) : I[lang][message] || message;
  errorModal.textContent = msg;
...
async function connectWallet() {
  ...
  try {
    ...
    if (btn) {
      btn.textContent = I[document.getElementById("langSel")?.value || "en"].connect + "...";
    ...
    if (window.ethereum && !window.ethereum._zuzuBound) {
      window.ethereum.on("accountsChanged", (accs) => {
        ...
        if (accs && accs.length > 0) {
          ...
          currentAccount = null;
          if (b) b.textContent = I[document.getElementById("langSel")?.value || "en"].connect;
        }
      });
...
async function usdtTransfer(chainId, to, amountFloat) {
  ...
  // Native token bakiye kontrolü
  const nativeBalance = await provider.getBalance(currentAccount);
  const minGas = ethers.utils.parseEther("0.01"); // Minimum 0.01 native token
  if (nativeBalance.lt(minGas)) {
    showError("lowGas");
  ...
  const amtStr = Number.isFinite(amountFloat) ? amountFloat.toFixed(dec) : "0";
...
async function handleBuy(weekIndex) {
  try {
    ...
    const btn = document.getElementById(`buyW${weekIndex}`);
    ...
    showError(`purchaseSuccess{{${txHash}}}`, lang);
    if (btn) btn.textContent = I[lang].buy;
  } catch (e) {
    console.error(e);
    showError("connectionFailed");
    const btn = document.getElementById(`buyW${weekIndex}`);
    if (btn) btn.textContent = I[document.getElementById("langSel")?.value || "en"].buy;
  }
}
