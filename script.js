...
========================= */
const CONFIG = {
  // EVM alanları artık kullanılmıyor; dokunmuyoruz
...
/* =========================================================
   ✅ SOLANA’YA GEÇİŞ — EVM/MetaMask kodları kaldırıldı
   - Week Buy butonları: USDT toplamını hesaplayıp Solana kutusuna yazar
   - Connect Wallet: solana.js içindeki __zuzuSolanaConnect() çağrılır
   - EVM "Network / MetaMask" kartı gizlenir
========================================================= */

// “Network” kartını ve MetaMask notunu gizle (HTML’i bozmadan)
(function hideEvmCard(){
  const netSel = document.getElementById("networkSel");
  if (!netSel) return;
  let card = netSel.closest(".card");
  if (card) card.style.display = "none";
  // Alttaki “Payments in USDT via MetaMask” metnini de sakla
  const note = card?.querySelector(".muted");
  if (note) note.style.display = "none";
})();

// Connect Wallet => Solana connect (solana.js sağlayacak)
(function wireSolanaConnect(){
  ...
  btn.addEventListener("click", ()=>{
    if (typeof window.__zuzuSolanaConnect === "function"){
      ...
      alert("Solana wallet not ready yet. Please refresh after scripts load.");
    }
  });
})();

// Hafta butonlarına tıklandığında: USDT maliyetini hesapla, Solana USDT alanına yaz, kullanıcıyı Solana kutusuna indir
function handleWeekClick(weekIndex){
  ...
  if (usdtInput){
    usdtInput.value = cost.toFixed(2);
  }
  const box = document.getElementById("solanaWalletBox");
  if (box && typeof box.scrollIntoView === "function"){
    box.scrollIntoView({behavior:"smooth", block:"center"});
  }
  // Solana USDT gönderim butonunu highlight et
  const usdtBtn = document.getElementById("btnBuyUSDT");
  if (usdtBtn){
    usdtBtn.classList.add("pulse");
    setTimeout(()=>usdtBtn.classList.remove("pulse"), 1200);
  }
}

// Week buy butonlarını Solana akışına bağla
["buyW0","buyW1","buyW2","buyW3"].forEach((id,i)=>{
  const b = document.getElementById(id);
  if (!b) return;
  b.onclick = ()=>handleWeekClick(i);
});
...
/* =========================
   UX küçük iyileştirmeler
========================= */
// Exchanges ticker görünürlüğü (mobilde ilk frame tetikleme)
...
// Solana tarafı bağlandığında nav’daki Connect buton etiketini güncellemek için global helper
window.__zuzuSetConnectLabel = function(text){
  const b = document.getElementById('connectBtn');
  if (b && text) b.textContent = text;
};

// Küçük highlight animasyonu için sınıf (style.css içinde varsa kullanırız; yoksa inline ekleyelim)
(function injectPulseStyle(){
  const id = "zuzu-pulse-style";
  if (document.getElementById(id)) return;
  const css = `.pulse{animation:zuzuPulse .6s ease-in-out 2}
@keyframes zuzuPulse{0%{transform:scale(1)}50%{transform:scale(1.05)}100%{transform:scale(1)}}`;
