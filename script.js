...
const CHAINS = {
  1:   { hex:"0x1",  name:"Ethereum", usdt:"0xdAC17F958D2ee523a2206206994597C13D831ec7", usdtDecimals:6,  params:null },
  56:  { hex:"0x38", name:"BNB Chain", usdt:"0x55d398326f99059fF775485246999027B3197955", usdtDecimals:18, params:{
  ...
  137: { hex:"0x89", name:"Polygon", usdt:"0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174", usdtDecimals:6, params:{
...
async function ensureProvider(){
  if (!window.ethereum) { alert("MetaMask not found. Please install MetaMask."); throw new Error("No MetaMask"); }
...
async function connectWallet(){
  ...
  if (btn && currentAccount) btn.textContent = `${currentAccount.slice(0,6)}...${currentAccount.slice(-4)}`;

  if (window.ethereum && !window.ethereum._zuzuBound) {
    window.ethereum.on("accountsChanged", (accs)=>{
      const b = document.getElementById("connectBtn");
      if (accs && accs.length>0) { currentAccount = accs[0]; if (b) b.textContent = `${currentAccount.slice(0,6)}...${currentAccount.slice(-4)}`; }
      else { currentAccount=null; if (b) b.textContent="Connect Wallet"; }
    });
...
async function usdtTransfer(chainId, to, amountFloat){
  await ensureProvider(); await connectIfNeeded();
...
async function handleBuy(weekIndex){
  try {
    ...
    console.error(e); alert("Transaction failed or rejected.");
  }
}
...
["buyW0","buyW1","buyW2","buyW3"].forEach((id,i)=>{
  const b = document.getElementById(id); if (b) b.addEventListener("click", ()=>handleBuy(i));
});
updateActiveWeekUI(); updateCosts();

/* === PATCH-C: helpers & small fixes === */
const ZUZU_CHAINS = {
  ETH:{chainId:'0x1', chainName:'Ethereum Mainnet', nativeCurrency:{name:'ETH',symbol:'ETH',decimals:18}, rpcUrls:['https://rpc.ankr.com/eth'], blockExplorerUrls:['https://etherscan.io']},
  ...
  POLY:{chainId:'0x89', chainName:'Polygon', nativeCurrency:{name:'MATIC',symbol:'MATIC',decimals:18}, rpcUrls:['https://polygon-rpc.com'], blockExplorerUrls:['https://polygonscan.com']}
};
async function zuzuSwitchChain(chain){
  if(!window.ethereum){ alert('MetaMask not detected'); return false; }
  const cfg = ZUZU_CHAINS[chain]; if(!cfg){ alert('Unknown chain'); return false; }
  try{
    await window.ethereum.request({ method:'wallet_switchEthereumChain', params:[{chainId:cfg.chainId}] });
    return true;
  }catch(e){
    if(e && e.code===4902){
      try{ await window.ethereum.request({ method:'wallet_addEthereumChain', params:[cfg] }); return true; }
      catch(e2){ console.warn('add chain rejected', e2); alert('User rejected chain add.'); return false; }
    }
    console.warn('switch chain failed', e); alert('Network switch failed.'); return false;
  }
}
...
(function guardConnectBtn(){
  const b = document.getElementById('connectBtn'); if(!b) return;
  b.addEventListener('click', ()=>{ if(!window.ethereum) alert('MetaMask not detected. Please install MetaMask and refresh the page.'); }, {capture:true});
})();
