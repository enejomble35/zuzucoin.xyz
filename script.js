...
const CONFIG = {
  ...
  // Countdown: 50 gün (ilk girişte hesapla ve localStorage'a sabitle)
  launchAt:
    Number(localStorage.getItem("zuzu_launchAt")) ||
    (function () {
      const t = Date.now() + 50 * 24 * 60 * 60 * 1000;
      localStorage.setItem("zuzu_launchAt", String(t));
      return t;
    })(),

  // Satış haftaları (başlangıç zamanı da sabit kalsın)
  saleStart:
    Number(localStorage.getItem("zuzu_saleStart")) ||
    (function () {
      const s = Date.now();
      localStorage.setItem("zuzu_saleStart", String(s));
      return s;
    })(),

  weekPrices: [0.005, 0.0065, 0.008, 0.01], // USDT
...
// Zincir/USDT adresleri ve ondalıklar
const CHAINS = {
  1: {
    hex: "0x1",
    name: "Ethereum",
...
(function setupCalc() {
  ...
  const calc = () => {
    const a =
      parseFloat((amount.value || "0").toString().replace(/[^\d.]/g, "")) || 0;
...
/* =========================================================
   MetaMask – EIP-6963 Discovery + Deep Link Fallback
========================================================= */
...
window.addEventListener(
  ...
  (event) => {
    ...
    // MetaMask'i önceliklendir
    if (p && p.isMetaMask) discoveredProvider = p;
    // başka cüzdanlar da olabilir
    if (!discoveredProvider && p) discoveredProvider = p;
  },
  { passive: true }
);
// Discovery'yi başlat
...
function getInjectedProvider() {
  return discoveredProvider || window.ethereum || null;
}

function openMetaMaskDeepLink() {
  // dapp link – MetaMask mobil iç tarayıcıda açar
  const clean = location.href.replace(/^https?:\/\//, "");
  const deep = `https://metamask.app.link/dapp/${clean}`;
  // yeni sekmede açmayı dene; olmazsa yönlendir
  const w = window.open(deep, "_blank");
  if (!w) location.href = deep;
}

async function ensureProvider() {
  const injected = getInjectedProvider();
  if (!injected) {
    // Mobil Chrome/Safari'de uzantı yoksa deep-link ile MetaMask'a taşı
  ...
  if (eth && !eth._zuzuBound) {
    eth.on("accountsChanged", (accs) => {
      ...
      if (accs && accs.length > 0) {
        ...
        if (b)
          b.textContent = `${currentAccount.slice(0, 6)}...${currentAccount.slice(-4)}`;
      } else {
        currentAccount = null;
...
async function switchNetwork(targetId) {
  ...
  try {
    ...
    if (err && err.code === 4902 && meta.params) {
      await eth.request({ method: "wallet_addEthereumChain", params: [meta.params] });
    } else {
      throw err;
    }
  }
...
async function usdtTransfer(chainId, to, amountFloat) {
  ...
  const amtStr = Number.isFinite(amountFloat)
    ? amountFloat.toFixed(dec > 6 ? 6 : dec)
    : "0";
...
async function handleBuy(weekIndex) {
  try {
    ...
    console.error(e);
    // Eğer provider yoksa deep-link dene
    if (String(e.message || "").includes("No injected provider")) {
      return; // deep link zaten açıldı
    }
...
document.getElementById("networkSel")?.addEventListener("change", async () => {
  try {
    await switchNetwork(getSelectedChainId());
  } catch (e) {
    console.warn(e);
  }
});
