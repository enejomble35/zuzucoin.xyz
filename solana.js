// solana.js — dış JS dosyası (script tag yok)
(function(){
  // İlk hali: libs gelmemişse stub uyarısı
  window.__zuzuSolanaConnect = function(){
    alert('Loading Solana wallet… please wait a moment and try again.');
  };

  // Dinamik script yükleyici (fallback ile)
  ...
  async function ensureLibs(){
    ...
    const tries = [
      ...
      // spl-token
      ["https://cdn.jsdelivr.net/npm/@solana/spl-token@0.4.6/lib/index.iife.min.js",
       "https://unpkg.com/@solana/spl-token@0.4.6/lib/index.iife.min.js"]
    ];
    for (const url of tries[0]) { if (!window.solanaWeb3) { try{ await loadScript(url); }catch(_){}} }
    for (const url of tries[1]) { if (!window.splToken) { try{ await loadScript(url); }catch(_){}} }
  }
  ...
  async function init(){
    ...
    const detectWallet = ()=>{
      // Phantom mobil: window.phantom?.solana veya window.solana
    ...
    const memoIx = (text)=> new TransactionInstruction({ keys:[], programId: MEMO_PROGRAM_ID, data: enc.encode(text) });
    const refString = ()=>{
    ...
    async function sendUSDT(){
      ...
      try{
        ...
        const amount = BigInt(Math.floor(amt * 1_000_000)); // 6 decimals
    ...
    // Gerçek connect fonksiyonunu yayınla
    ...
    if (w && !w._zuzuBound && w.on){
      w.on('accountChanged', (pk)=>{
        if(pk){ pubkey=new PublicKey(pk); setConnectedUI(true); setStatus('Hesap değişti'); if (typeof window.__zuzuSetReferral==='function') window.__zuzuSetReferral(pubkey.toBase58()); }
        else { disconnect(); }
      });
      w._zuzuBound=true;
    }
